...
// The model that powers every Anthropic call in this file (analysis and the
// Health Check re-score) — the ONLY place this string should ever appear.
const MODEL = "claude-opus-4-8";

// Shared between EXTRACTION_PROMPT (scoring a fresh case) and
// HEALTH_CHECK_PROMPT (re-scoring an already-analyzed, possibly hand-edited
// case) so the rubric can't drift between the two call sites.
const HEALTH_CHECK_RUBRIC = `  PAIN: 0 none·1 admitted·2 documented pain+causes confirmed·3 discussed with power·4 pain flow explored org-wide·5 power committed to a time-based reason to act·6 power confirms link to strategy.
...
MILESTONE CONTEXT (ground value and competitive positioning here; NEVER invent products or numbers): open-platform VMS XProtect (editions Essential+ through Corporate), BriefCam video analytics (forensic smart-search, real-time behavioural alerts), Arcules cloud/hybrid VSaaS, Milestone Interconnect for linking independent multi-site XProtect systems, XProtect extensions (Access, LPR, Transact, Smart Wall, Incident Manager, Hospital Assist), ~13,000 supported devices via broad device-driver library, ~3,500 technology partners, sold and delivered through a certified reseller/integrator channel, Milestone Care/Care Plus support. Differentiators must map to these real capabilities; parity claims must be genuine parity; never claim a capability Milestone does not have.

METHODOLOGY:
- Pain follows Milestone's Validation Communication structure: Pain Description -> Pain -> Causes -> Capabilities. These are two DIFFERENT fields — never blend them:
  painDescription = the neutral, factual situation: what is broken, missing or failing, stated without consequence or judgment. Just the observable fact.
  pain = ONLY the consequence of that fact, built strictly from the MEDDIC pain criteria — Personal (a named role/title owns it — is measured, held accountable, or answers for it), Measurable (quantify the impact wherever the case supports a number — cost, time, headcount, fines; when a genuine dollar figure isn't supportable, quantify in the next-best concrete unit — regulatory exposure, safety/life-risk, reputational/contractual consequence — never fall back to vague language just because there's no dollar figure), Negatively stated, AND explicitly links to company strategy or a named business consequence. pain must NOT restate the fact already given in painDescription — it starts from that fact and states only what it costs, who answers for it, and why it matters strategically.
  Each pain has an OWNER (may differ from the role named inside the pain statement, e.g. a team that tracks the exposure). Causes are the underlying reasons for the fact in painDescription — never phrase as "they lack X" or "they need X". Capabilities are Issue/Action/Value statements addressing the cause and must NOT name specific products; capabilities must reuse the specific mechanism named in causes — a capability generic enough to fit any cause is wrong.
...
- Value = quantified drivers + Value Statements (Issue/Action/Value/Check). Each quantified driver follows Metric/KPI -> Baseline -> Target -> $ Impact: name the specific business metric, its current measured (or realistically inferred) value, the target value once Milestone's solution is in place, and the resulting quantified impact (dollars, hours, headcount or risk-cost avoided). Baseline and target must use the SAME unit so the gap is legible at a glance.
...
- Competitive = Value Grid: every differentiator scores uniqueness (0-10, vs the named competitor) and customerValue (0-10); differentiators[0] must score high on both (the "Differentiators" quadrant — high uniqueness + high value). Lead with unique differentiators, prove parity elsewhere. differentiatorPool = 3 MORE capabilities, same shape and same grounding rules as differentiators, but genuinely different capabilities (not rephrasings of the 4 above) — this feeds a "reroll" in the trainee UI that swaps in a fresh set of 4 to classify on the Value Grid. Across the combined 7 (differentiators + pool), at least 2 must genuinely land in the "Cool stuff" quadrant (uniqueness 6-9, customerValue 2-4) — real, case-relevant capabilities that sound impressive but don't move THIS deal (wrong scale, wrong budget tier, or a requirement the case doesn't actually have), so a rep who reflexively leads with the flashiest-sounding capability gets caught; put these in differentiatorPool, never in differentiators[0-3]. The rest should spread across Core/Trivial too, not cluster in "Differentiators".
- Objection handling: the 2 objections must read as something a SALES rep handles solo, not a commercial-only negotiation drill. At least one of the 2 must be a SOLUTION-FIT objection — the customer doubting whether Milestone's Platform, an Extension, a Technology Partner, or a Device Integration actually covers a stated requirement — named at the layer/capability level (e.g. "XProtect Access", "BriefCam", a named partner category) so the rep can point to the right layer, but never precise technical mechanics, protocols, or configuration steps — that depth is the pre-sales engineer's job, not the seller's. The other objection may stay commercial/negotiation (price, timeline, procurement, vendor consolidation) or also be solution-fit; either way Position stays business-level, grounded in this case's actual requirements.
...
- solution = the open-platform solution map for THIS case, layered like Milestone's architecture slide. Pick ONLY from these canonical catalogs — never invent an item; leave a layer short (or empty for extensions/analytics/cloud) if the case genuinely doesn't need it. The platform layer is the DELIBERATE exception: list EVERY native capability the case's requirements genuinely involve — aim for breadth here, mining the whole platform catalog — while every other layer stays tight. Still never list a platform capability the requirements don't actually touch. Each item's purpose ties it to a specific requirement or pain of this case in ≤ 20 words.
  ...
  platform catalog (native XProtect — from the 2026 R1 product range; pick every group this case's requirements touch, and be generous within those groups): Architecture & servers — Recording Server, Management Server, Event Server, Centralized Multi-Server Management, Distributed Architecture, High Availability / Failover Support, Failover Event Server, Milestone Federated Architecture, Remote Connection Service, AWS Cloud Deployment. Device handling — ONVIF/PSIA & Universal Driver Support (11,000+ devices), Secure HTTPS Camera Connectivity, Single & Bulk Device Firmware Upgrade, Device Scanning & Model Detection, Device Replacement Wizard, Move Device Wizard, Device Password Management. Video & streaming — Multi-Codec Support (H.264/H.265/AV1/MJPEG/MPEG-4/MxPEG), Dual Streaming, Adaptive Streaming/Playback, Direct Streaming, Multi-Live Streaming, Generic 360° Dewarp, Scalable Video Quality Recording, Multicast. Motion & metadata — Built-in Video Motion Detection (auto-adjustable sensitivity, exclusion zones), Motion Metadata, Camera Embedded-Analytics Metadata Support, Hardware Accelerated Decoding (Quick Sync / NVIDIA). Audio — One-way & Two-way Audio (live + recording), AAC Audio, Audio Playback on Rule. PTZ — PTZ Presets & Patrolling, Go-to-Preset on Event, Prioritized PTZ Control. Events & automation — Flexible Event Rule Engine, I/O Events & Alarm Management, System/Manual/Generic Events, Event-Based Output & Device Control, Event-Based Notification, Time Profiles. Recording & storage — Pre-Recording Buffer, Unrestricted Retention, Archiving (schedules + network drives), Edge Storage Retrieval, Multi-Stage Storage & Data Grooming, Media Database Encryption & Digital Signing. Alarm Manager — Role-Based Alarm Handling, Alarm Priorities/Categories/Statuses, Alarm Work Instructions, Alarm Reporting & Logging, Push Alarms to Mobile. Evidence & bookmarks — Evidence Lock, Video Restrictions, Manual & Rule-Based Bookmarking. Maps — Map Function, Smart Map (GIS / OpenStreetMap / Bing / Google), Device Control on Maps. Integration (MIP) — MIP SDK & REST API, WebRTC Streaming, WebSocket Event/Alarm Integration, Webhooks, Driver Framework, Milestone AI Bridge, HID Integration. Security & identity — Communication Encryption (servers/clients/integrations), FIPS 140-2 Compliance, Centralized User Management, Active Directory, External IdP (OIDC SSO) + SCIM Provisioning, Dual Authorization, Two-Step Verification, Tiered Management Rights, Time-Scheduled Access, Kerberos/NTLM, Privacy Masking. Monitoring & management — System Monitor (real-time & historical health, thresholds), Configuration Reporting, Backup/Restore, Management Client Profiles, Smart Client Profiles, Customer Dashboard & Connected Services. Client access — XProtect Smart Client / Web Client / Mobile, Milestone Open Network Bridge, Centralized Search (Sequence/Motion/Alarm/Event), Evidence Export & Storyboarding.
...
SPECIFICITY: every pain/cause/capability/value string must anchor to a concrete noun from the document or a realistic inferred one — a system name, a number, a role, a metric, a location. Never a generic phrase ("improves efficiency", "streamlines operations", "enhances security") without saying what improves and by roughly how much. If a sentence would read the same for any customer in any industry, rewrite it.

CALIBRATION EXAMPLE (match this level of specificity and register — do not reuse its content):
  painDescription: "The Director of Security cannot approve access-list changes without physically visiting each site, delaying critical revocations by up to 48 hours."
  pain: "Security operations owns this exposure, and the resulting audit-flagged compliance gap — tracked by the risk and compliance team — undermines the site's certified-access-control posture."
  ...
  Value driver — metric: "Average access-revocation turnaround time" baseline: "48 hours across 6 sites, per the Q3 security log" target: "under 2 hours with a centralized console" impact: "$140K/year in avoided audit-remediation and overtime costs."
...
REQUIREMENTS: rfi.sections mirror the document's own numbering/titles (e.g. "2.7 Degraded Mode") and hold only the facts of each requirement; mark hard requirements critical:true. solution.narrative is 1-2 sentences on how the layered solution answers this case; most solution layers hold 2-6 items each, but the platform layer is comprehensive — list every native capability the requirements involve (typically 6-12, more when the case warrants it); deviceIntegration and platform are never empty; extensions/analytics/cloud may be empty when genuinely irrelevant. Do NOT add a Milestone response in the Overview. 5-6 pains. 6-8 vision items. 5-6 value drivers, each with a baseline and target in the same unit plus a quantified $ impact. exactly 3 value statements (action starts "Imagine…"/"Consider…"). 4-6 collaboration-plan events spanning Solution/Transition/Financial phases, at least one marked goNoGo:true. differentiators[0] is the demo centerpiece (solves the hardest requirement, uniqueness 8-10 and customerValue 8-10). pains[].affects is empty only if the case genuinely has one isolated stakeholder. 2 fully-worked objections (at least one solution-fit, per the Objection handling rule above). 3 red flags. If no competitor is named, infer the most likely one. healthCheck scores are always src:"doc"-grade judgment calls, not inferred facts — each rationale is one sentence citing what is (or isn't) evidenced in the document.

WORD BUDGETS (compact ≠ vague — use the budget to be specific, don't pad to fill it, don't cut a concrete detail to save words): titles, labels, category names, discovery statements, value driver metric/baseline/target/impact ≤ 15 words each. summary, painHeadline, narrative, healthCheck rationale, rfi section summaries, painDescription, causes, capabilities ≤ 30 words. pain (states only the consequence — owner accountability, quantified impact, strategic link — never the fact already in painDescription) ≤ 40 words. value statement fields (issue/action/value/check), objection fields (acknowledge/question/position/check), differentiator/red-flag detail ≤ 50 words. This keeps the analysis within the output budget — do not exceed these caps or generation may run out of tokens mid-call.

The user message contains the customer case to analyse.`;

// Re-scores the Health Check block only, against a case JSON that already
// exists (and may have been hand-edited by a trainer since the original
// analysis) — shares HEALTH_CHECK_RUBRIC with EXTRACTION_PROMPT so the two
// scoring passes can never drift apart.
const HEALTH_CHECK_PROMPT = `You are a sales-enablement analyst at Milestone Systems, re-scoring the Opportunity Health Check for an existing Growth Activator case briefing. The seller may have edited the case since it was first analyzed — score the case JSON exactly as given, not any prior version.

Score PAIN, POWER, VISION, VALUE, CONSENSUS 0-6 using Milestone's rubric below, based ONLY on what the case JSON evidences about the sales process itself (not the customer's business situation). A case briefing rarely documents live deal history — most scores will be 0-2. Do NOT invent meetings, agreements or collaboration the JSON doesn't describe. Each rationale is one sentence citing what is (or isn't) evidenced in the case, ≤ 30 words.

${HEALTH_CHECK_RUBRIC}

The user message contains the full case JSON to score.`;
...
// Shared between CASE_ANALYSIS_SCHEMA (full analysis) and the standalone
// emit_health_check tool used to re-score an existing case.
  ...
  properties: {
    ...
    pains: {
      ...
      items: {
        ...
        required: ["title", "category", "owner", "painDescription", "pain", "causes", "capabilities", "affects", "src"],
        additionalProperties: false,
      },
    },
      ...
      properties: {
        drivers: {
          ...
          items: {
            ...
            required: ["metric", "baseline", "target", "impact", "src"],
            additionalProperties: false,
          },
        },
        statements: {
    ...
    competitive: {
      ...
      required: ["narrative", "differentiators", "differentiatorPool", "parity", "objections", "redFlags"],
      additionalProperties: false,
    },
    healthCheck: {
      type: "object",
...
// Re-scores the Health Check block for an already-analyzed case. Unlike
// analyzeCase this is a single blocking call (no streaming) — the rubric
// pass is short enough that a spinner beats the complexity of SSE here.
async function rerunHealthCheck(caseFile) {
      ...
      tools: [
        {
          ...
          description: "Return the re-scored Opportunity Health Check.",
          input_schema: HEALTH_CHECK_SCHEMA,
        },
      ],
      tool_choice: { type: "tool", name: "emit_health_check" },
      system: [{ type: "text", text: HEALTH_CHECK_PROMPT }],
      messages: [{ role: "user", content: "CASE JSON:\n" + JSON.stringify(caseFile) }],
    })
  );
  ...
  const toolUse = data.content?.find((b) => b.type === "tool_use" && b.name === "emit_health_check");
  if (!toolUse) throw new Error("Model did not call the emit_health_check tool.");
...
// Re-score Health Check for an existing (possibly edited) case
app.post("/api/health-check", asyncHandler(async (req, res) => {
  const { caseFile } = req.body;
  if (!caseFile) return res.status(400).json({ error: "caseFile required" });
